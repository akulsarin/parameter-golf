...
class Hyperparameters:
    ...
    train_seq_len: int = int(
        os.environ.get(
            "TRAIN_SEQ_LEN", os.environ.get("TRAIN_MAX_SEQ_LEN", 1024)
        )
    )
    ...
    max_wallclock_seconds: float = float(
        os.environ.get("MAX_WALLCLOCK_SECONDS", 600.0)
    )
    ...
    tied_embed_init_std: float = float(
        os.environ.get("TIED_EMBED_INIT_STD", 0.005)
    )
    ...
    def lr_mul(self, step: int, elapsed_ms: float) -> float:
        ...
        remaining_ms = max(
            1000.0 * self.max_wallclock_seconds - elapsed_ms, 0.0
        )
        return (
...
def token_chunks(
    total_tokens: int, seq_len: int, max_chunk_tokens: int
) -> list[int]:
...
def rms_norm(x: mx.array, eps: float = 1e-6) -> mx.array:
    return (x * mx.rsqrt(mx.mean(x * x, axis=-1, keepdims=True) + eps)).astype(
        x.dtype
    )
...
def load_data_shard(path: Path) -> np.ndarray:
    ...
    tokens = np.fromfile(
        path, dtype="<u2", count=num_tokens, offset=header_bytes
    )
...
class TokenLoader:
    def __init__(
        ...
        self.stream = TokenStream(
            pattern, log_fn=log_fn, dataset_name=dataset_name
        )

    def next_batch(
        self, batch_tokens: int, seq_len: int
...
class CastedLinear(nn.Module):
    def __init__(self, in_dim: int, out_dim: int):
        super().__init__()
        self.weight = nn.Linear(in_dim, out_dim, bias=False).weight.astype(
            mx.float32
        )
...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        self.skip_weights = mx.ones(
            (self.num_skip_weights, dim), dtype=mx.float32
        )
        self.blocks = [
            Block(
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        for i in range(self.num_decoder_layers):
            ...
            if skips:
                x = (
                    x
                    + self.skip_weights[i].astype(x.dtype)[None, None, :]
                    * skips.pop()
                )
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        ...
        if (
            ...
            or x.shape[0] <= self.logit_chunk_tokens
        ):
...
class Muon:
    ...
    def step(
        ...
        if self.args.muon_momentum_warmup_steps:
            ...
            momentum = (
                (1.0 - t) * self.args.muon_momentum_warmup_start
        ...
        for k in self.keys:
            ...
            g_ortho = zeropower_newtonschulz5(
                g_eff, self.args.muon_backend_steps
            )
...
class SplitOptimizers:
    ...
    def __init__(self, model: GPT, args: Hyperparameters):
        ...
        self.matrix_keys = [
            ...
            and p.ndim == 2
            and not any(
                pattern in k for pattern in CONTROL_TENSOR_NAME_PATTERNS
            )
        ]
        self.scalar_keys = [
            k
    ...
    def step(
        self, model: GPT, grads_tree: dict, step: int, lr_mul: float
    ) -> None:
        params = dict(tree_flatten(model.parameters()))
...
def keep_float_array(
    ...
    if arr.dtype in {mx.float32, mx.bfloat16}:
        ...
        return np.ascontiguousarray(
            np.array(
                arr.astype(mx.float16),
                dtype=INT8_KEEP_FLOAT_STORE_DTYPE,
                copy=False,
            )
        )
...
def quantize_float_array(arr: mx.array) -> tuple[np.ndarray, np.ndarray]:
    ...
    if f32.ndim == 2:
        ...
        scale = np.maximum(clip_abs / 127.0, 1.0 / 127.0).astype(
    ...
    clip_abs = (
        float(np.quantile(np.abs(f32).reshape(-1), INT8_CLIP_Q))
        if f32.size
        else 0.0
    )
    scale = np.array(
        clip_abs / 127.0 if clip_abs > 0.0 else 1.0, dtype=np.float32
    )
    q = np.clip(
...
def validate_dataset_tokenizer_pair(
    ...
    dataset_entry = next(
        (
            ...
            if x.get("name") == dataset_dir.name
        ),
        None,
    )
...
def clip_grad_tree(grads_tree: dict, max_norm: float) -> dict:
    ...
    for grad in flat.values():
        total_sq += float(
            np.sum(np.square(_np_float32(grad)), dtype=np.float64)
        )
    if total_sq <= 0.0:
        return grads_tree
...
def main() -> None:
    ...
    if not args.tie_embeddings:
        raise NotImplementedError(
            "train_gpt_mlx.py only supports tied embeddings"
        )
    ...
    n_params = sum(
        int(np.prod(p.shape)) for _, p in tree_flatten(model.parameters())
    )
    ...
    while True:
        ...
        approx_train_time_ms = train_time_ms + 1000.0 * (
